...
int guild_reqalliance(struct map_session_data *sd,struct map_session_data *tsd)
{
	...
	if( guild_get_alliance_count(g[0],0)>=battle_config.guild_allience )	{
		clif_guild_allianceack(sd,4);
		return 0;
	}
	if( guild_get_alliance_count(g[1],0)>=battle_config.guild_allience ) {
...
int guild_reply_reqalliance(struct map_session_data *sd,int account_id,int flag)
{
	...
	if(flag==1){	// 承諾
		...
		if(g==NULL || guild_get_alliance_count(g,0)>=battle_config.guild_allience){
			clif_guild_allianceack(sd,4);
			clif_guild_allianceack(tsd,3);
			return 0;
		}
		if(tg==NULL || guild_get_alliance_count(tg,0)>=battle_config.guild_allience){
			clif_guild_allianceack(sd,3);
