...
TBL_PC *script_rid2sd(struct script_state *st)
{
	...
	if(!sd){
		...
		ShowError("Because you use attachrid(player_id); command");
		st->state = END;
	}
	return sd;
}
		...
		case C_FUNC:
			...
			if(st->state==GOTO){
				...
				if( gotocount>0 && (--gotocount)<=0 ){
					...
					ShowError("Because you use goto 'Label'; function");
...
BUILDIN_FUNC(delpet); //nznick
BUILDIN_FUNC(createpet); //nznick
BUILDIN_FUNC(writetofile); //nznick
BUILDIN_FUNC(readfromfile); //nznick
...
struct script_function buildin_func[] = {
	BUILDIN_DEF(writetofile,"ss"), //nznick
	BUILDIN_DEF(readfromfile,"s"), //nznick
	BUILDIN_DEF(createpet,"i"),
	BUILDIN_DEF(delpet,""),
	// NPC interaction
	BUILDIN_DEF(mes,"s"),
	BUILDIN_DEF(next,""),
	...
	if(p!=NULL){
		for(i=0;i<MAX_PARTY;i++){
			if(p->party.member[i].account_id){
				switch (type) {
				case 4:
					mapreg_setreg(add_str("$@partymemberclass")+(j<<24),p->party.member[i].class_);
					break;
				case 3:
					mapreg_setreg(add_str("$@partymemberlv")+(j<<24),p->party.member[i].lv);
					break;
				case 2:
...
BUILDIN_FUNC(jump_zero)
{
	...
	if(!sel) {
		...
		if( !data_islabel(script_getdata(st,3)) ){
			...
			ShowError("Because you use jump_zero command to point label which not have already!");
			st->state=END;
			return 1;
		}
...
BUILDIN_FUNC(writetofile)
{
	//char buffer[2048],path[2048],text[2048];
	FILE *fp = fopen(script_getstr(st,2),"w+t");
	if(fp==NULL) {
		script_pushint(st,1);
		return 1;
	}

	fputs(script_getstr(st,3),fp);
	script_pushint(st,0);
	fclose(fp);
	return 0;
}

BUILDIN_FUNC(readfromfile)
{
	char buffer[2048];
	FILE *fp = fopen(script_getstr(st,2),"r+t");
	if(fp==NULL) {
		script_pushint(st,1);
		return 1;
	}
...
BUILDIN_FUNC(createpet)
{
	/*int pet_ids = script_getnum(st,2);
	TBL_PC *sd = script_rid2sd(st);
	//struct s_pet *pet;
	...
	sd->pd->pet.pet_id = pet_ids;
	pet_birth_process(sd,&sd->pd->pet);
	//if(!pet_change(sd,&sd->pd->pet,pet_ids))
	//	return 0;
	/*///struct s_pet *pet;
