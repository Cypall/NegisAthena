...
void vending_openvending(struct map_session_data* sd, const char* message, bool flag, const uint8* data, int count)
{
	...
	if(map_getcell(sd->bl.m,sd->bl.x,sd->bl.y,CELL_CHKNOVENDING)!=0) {
		//clif_displaymessage(sd->fd,"You Can't vending in this area.");
		clif_displaymessage(sd->fd,msg_txt(403));
		return;
	}
