...
int enable_read_scache = 0; // 0=disable, 1=enable
...
void map_setcell(int m,int x,int y,int cell)
{
	...
	switch (cell) {
		...
		case CELL_SETREGEN:
			...
			break;
		case CELL_SETNOVENDING:
			map[m].cell[j] |= CELL_NOVENDING;
			break;
		default:
			map[m].gat[j] = cell;
			break;
	...
	while(fgets(line, sizeof(line), fp))
	{
		...
		if (sscanf(line, "%[^:]: %[^\t\r\n]", w1, w2) == 2) {
			...
			*ptr = '\0';
			...
			} else if (strcmpi(w1, "enable_read_scache") == 0) {
				enable_read_scache = atoi(w2);
...
int do_init(int argc, char *argv[])
{
	...
	if (battle_config.pk_mode == 1)
		ShowNotice("Negis is running on '"CL_WHITE"PK Mode"CL_RESET"'.\n");

	ShowStatus("Negis is '"CL_GREEN"ready"CL_RESET"' and listening on port '"CL_WHITE"%d"CL_RESET"'.\n\n", map_port);
