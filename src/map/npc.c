...
void npc_cachescript(const char* name)
{
	char ch;
	FILE *fp = fopen(name,"r");
	FILE *fp2 = fopen("npc/script_cache.txt","w+b"); 
	if( fp == NULL)
		return;
	while(!feof(fp))
	{
...
{
	char str[20] = "/0";
	if(remove("npc/script_cache.bin")!=-1)
		ShowInfo("Cleared Cache\n");

	//FILE *fp = fopen("npc/script_cache.bin","w+t");
	//if(fp==NULL)
	//	fputs(str,fpx);
	//fclose(fp);
}

void npc_cacheappend(const char* name)
{
	char ch[4096];//[2048];
	//char ch2[2048];
	//int len;
	FILE *fp = fopen(name,"r");
	FILE *fp2 = fopen("npc/script_cache.bin","a+t"); 
	if( fp == NULL)
		return;
	while(!feof(fp))
	{
...
void npc_parsedeffile(const char* name)
{
	/*char line[2048];
	char current_files[255];
	FILE *fp = fopen (name,"r+t");
	FILE *fp2;
	if(fp == NULL) {
		ShowError ("Define file not found : %s\n",name);
		return;
	}
	fp2 = fopen ("save/defreg.txt","a+t");
	while(!feof(fp))
	{
		char w1[2048],w2[2048],w3[2048];
		fgets(line,sizeof(line),fp);
		sscanf(line,"%s %s %s",w1,w2,w3);
		if(w1 == "#define") {
			if(w2[strlen(w2-1)] == "$") {
				if(w3 == "") {
					ShowError("Not have #define %s string values!",w2);
					continue;
				}
				fprintf(fp2,"%s %s %s",w1,w2,w3);
			} else {
				if(w3 == "") {
					ShowError("Not have #define %s number values!",w2);
					continue;
				}
				fprintf(fp2,"%s %s %s",w1,w2,w3);
			}
	...
	while (fgets(line, sizeof(line), fp))
	{
		...
		if ((count = sscanf(line, "%[^\t\r\n]\t%[^\t\r\n]\t%[^\t\r\n]\t%n%[^\r\n]", w1, w2, w3, &w4pos, w4)) < 3)
		{
			...
			(count = sscanf(line, "%s %s %s %n%[^\n]\n", w1, w2, w3, &w4pos, w4)) >= 3)
			{
				...
				if(name = "npc/script_cache.bin") continue;
...
int npc_reload(void)
{
	...
	if(enable_read_scache==1)
		npc_flushcache();

	// Reloading npcs now
	for (nsl = npc_src_files; nsl; nsl = nsl->next)
	{
		if(enable_read_scache==0) {
			ShowStatus("Loading NPC file: %s"CL_CLL"\r", nsl->name);
			npc_parsesrcfile(nsl->name);
		} else if(enable_read_scache==1) {
			ShowStatus("Caching NPC file: %s"CL_CLL"\r", nsl->name);
			npc_cacheappend(nsl->name);
		} else
			break;
	}
	if(enable_read_scache==1) npc_parsesrcfile("npc/script_cache.bin");
	if(enable_read_scache>1) npc_parsesrcfile("npc/script_cache.bin");
...
int do_init_npc(void)
{
	...
	for( file = npc_src_files; file != NULL; file = file->next )
	{
		if(enable_read_scache==0) {
			...
			ShowStatus("Caching NPC file: %s"CL_CLL"\r", file->name);
			npc_cacheappend(file->name);
		} else
			break;
	}
	/*fp = fopen("define_var.bin","r+t");
	if(fp != NULL) {
		npc_parsedeffile("define_var.bin");
		fclose(fp);
	} else {
		ShowWarning("Not have define variable files!");
	}*/
	if(enable_read_scache==1) { 
		npc_parsesrcfile("npc/script_cache.bin"); 
	} else if(enable_read_scache>1) { 
		npc_parsesrcfile("npc/script_cache.bin"); 
	}
	...
	//ShowInfo ("Done Loading '"CL_WHITE"%d"CL_RESET"' NPCs"CL_CLL,npc_id - START_NPC_NUM);
	printf("\n");
